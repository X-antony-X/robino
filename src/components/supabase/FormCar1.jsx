...
import { uploadCar1Images } from "./uploadCar1Images";
import { addToCar1 } from "./AddToCar1";
import { useState , useRef} from "react";
import { deleteCar1 } from "./deleteCar1";
...
function FormCar1() {
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            const imageUrls = await uploadCar1Images(files);
            return addToCar1({ title, images: imageUrls });
        },
        onSuccess:() => {
            queryClient.invalidateQueries({queryKey : ["carousel1"]})
            setTitle("")
            if (fileRef.current) {
    ...
    return (
        <div className="w-full flex justify-center">
            <div className="w-full max-w-md px-4 pt-10">
                ...
                before:bg-gradient-to-b before:from-slate-50/10 before:blur-xl">
                CAROUSEL 1
                </p>
