...
import { uploadCar2Images } from "./uploadCar2Images";
import { addToCar2 } from "./AddToCar2";
import { useState , useRef} from "react";
import { deleteCar2 } from "./deleteCar2";
...
function FormCar2() {
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            const imageUrls = await uploadCar2Images(files);
            return addToCar2({ title, images: imageUrls });
        },
        onSuccess:() => {
            queryClient.invalidateQueries({queryKey : ["carousel2"]})
            setTitle("")
            if (fileRef.current) {
