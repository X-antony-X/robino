...
import { uploadCar4Images } from "./uploadCar4Images";
import { addToCar4 } from "./AddToCar4";
import { useState , useRef} from "react";
import { deleteCar4 } from "./deleteCar4";
...
function FormCar4() {
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            const imageUrls = await uploadCar4Images(files);
            return addToCar4({ title, images: imageUrls });
        },
        onSuccess:() => {
            queryClient.invalidateQueries({queryKey : ["carousel4"]})
            setTitle("")
            if (fileRef.current) {
